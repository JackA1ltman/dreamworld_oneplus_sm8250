...
struct inode *ntfs_alloc_big_inode(struct super_block *sb)
{
	...
	ntfs_debug("Entering.");
#if 0
	ni = alloc_inode(/* was alloc_inode_sb */sb, ntfs_big_inode_cache, GFP_NOFS);
#else
...
int ntfs_read_inode_mount(struct inode *vi)
{
	...
	if (err) {
		...
		ni->attr_list = kvzalloc(round_up(ni->attr_list_size, 512),
					 GFP_NOFS);
		if (!ni->attr_list) {
...
#else
		aln = krealloc(al, al_len - ale_size, al_len, GFP_NOFS);
#endif
		if (!aln) {
			err = -ENOMEM;
...
int ntfs_inode_close(struct ntfs_inode *ni)
{
	...
	for (i = 0; i < base_ni->nr_extents; ++i) {
		...
		if (base_ni->nr_extents) {
			...
			tmp_nis = krealloc(tmp_nis, base_ni->nr_extents *
...
s64 ntfs_inode_attr_pread(struct inode *vi, s64 pos, s64 count, u8 *buf)
{
	...
	do {
		...
		attr_len = min_t(size_t, (size_t)count, PAGE_SIZE - offset);

		lock_page(page);
		memcpy_from_page(buf, folio, offset, attr_len);
		unlock_page(page);
		put_page(page);
					     ...
					     struct ntfs_attr_search_ctx *ctx)
{
	...
	if (!folio_test_uptodate(folio)) {
		...
		memcpy_to_page(folio, 0, addr, len);
		zero_user_segment(page, offset_in_page(len),
				   PAGE_SIZE - len);
#endif
	} else {
		memcpy_to_page(folio, offset_in_page(pos), buf, count);
	}
						 ...
						 bool sync)
{
	...
	while (count) {
		...
		memcpy_to_page(folio, offset, buf, attr_len);
		...
		if (sync) {
			...
			do {
				...
				length = min_t(unsigned long,
					       ...
					       PAGE_SIZE);
				if (!bio_add_page(bio, folio, length, offset)) {
					ret = -EIO;
					bio_put(bio);
					goto err_unlock_folio;
				}
