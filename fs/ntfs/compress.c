 ...
 */
#if 0
int ntfs_read_compressed_block(struct page *folio)
{
		...
		struct iov_iter *from)
{
	...
	while (count) {
		...
		for (i = 0; i < pages_per_cb; i++) {
			fpage = read_mapping_page(mapping, index + i, NULL);
			if (IS_ERR(fpage)) {
				for (ip = 0; ip < i; ip++) {
					unlock_page(pages[ip]);
					put_page(pages[ip]);
				}
				err = PTR_ERR(fpage);
				goto out;
		...
		for (;;) {
			...
			page = pages[ip];
#if 0
			cp = copy_folio_from_iter_atomic(page, off,
					min(tail, bytes), from);
#else
