...
static int ntfs_file_open(struct inode *vi, struct file *filp)
{
	...
	filp->f_mode |= FMODE_NOWAIT | 0 /* FMODE_CAN_ODIRECT */;
...
#endif
{
	 ...
	 * logical block size.
	 */
	/* DIO alignment not reported in 4.19 */
	if (0 && S_ISREG(inode->i_mode))
		;
...
/* iomap_dio_ops not in 4.19 - end_io passed directly to iomap_dio_rw */
...
static int ntfs_ioctl_fitrim(struct ntfs_volume *vol, unsigned long arg)
{
	...
	range.minlen = max_t(unsigned int, range.minlen, bdev_get_queue(dev)->limits.discard_granularity);
...
static long ntfs_fallocate(struct file *file, int mode, loff_t offset, loff_t len)
{
	...
	if (mode & (FALLOC_FL_PUNCH_HOLE | FALLOC_FL_COLLAPSE_RANGE |
		    FALLOC_FL_INSERT_RANGE)) {
		/* filemap_invalidate_lock not in 4.19 */
		map_locked = true;
	}
...
out:
	if (map_locked)
		/* filemap_invalidate_unlock not in 4.19 */
	if (!err) {
