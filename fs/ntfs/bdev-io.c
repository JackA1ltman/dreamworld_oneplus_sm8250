...
int ntfs_dev_read(struct super_block *sb, void *buf, loff_t start, size_t size)
{
	...
	for (; idx < idx_end; idx++, from = 0) {
#if 0
		folio = read_mapping_page(sb->s_bdev->bd_mapping, idx, NULL);
#else
		folio = read_mapping_page(sb->s_bdev->bd_inode->i_mapping, idx, NULL);
#endif
		if (IS_ERR(folio)) {
		...
		memcpy_from_page(buf + buf_off, folio, from, to);
		buf_off += to;
		put_page(page);
	}
...
int ntfs_bdev_write(struct super_block *sb, void *buf, loff_t start, size_t size)
{
	...
	for (; idx < idx_end; idx++, from = 0) {
		...
		memcpy_to_page(folio, from, buf + buf_off, len);
		buf_off += len;
