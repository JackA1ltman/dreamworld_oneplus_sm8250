...
static const struct fs_parameter_enum ntfs_param_enums[] = {
...
static const struct fs_parameter_description ntfs_parameters = {
	.name		= "ntfs",
	.specs		= ntfs_param_specs,
	.enums		= ntfs_param_enums,
};
...
static bool check_mft_mirror(struct ntfs_volume *vol)
{
...
#if 0
	struct page *mft_folio = NULL, *mirr_folio = NULL;
#else
	...
	do {
		...
		if (!(i % mrecs_per_page)) {
			...
			/* Get the $MFT page. */
			mft_folio = read_mapping_page(vol->mft_ino->i_mapping,
					index, NULL);
			if (IS_ERR(mft_folio)) {
			...
			mirr_folio = read_mapping_page(vol->mftmirr_ino->i_mapping,
					index, NULL);
...
#if 0
	folio = read_mapping_page(vi->i_mapping, 0, NULL);
	if (IS_ERR(folio)) {
...
#if 0
		folio = read_mapping_page(ino->i_mapping, index, NULL);
		if (IS_ERR(folio))
...
static int ntfs_statfs(struct dentry *dentry, struct kstatfs *sfs)
{
	 ...
	 * 32-bits in f_fsid[1].
	 */
	sfs->f_fsid.val[0] = (int)vol->serial_no;
	sfs->f_fsid.val[1] = (int)(vol->serial_no >> 32);
...
static int ntfs_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	if (NVolDiscard(vol) && !bdev_get_queue(sb->s_bdev)) {
		ntfs_warning(
			sb,
