...
#include "zxc_deps.h" /* kernel deps: slab allocator macros, missing libc constants */

/* Linux-kernel build of the public API surface (constants, error codes, option
 * structs, block / context / static-context prototypes).  Upstream this was
 * zxc_buffer.h + zxc_constants.h + zxc_error.h + zxc_opts.h + zxc_seekable.h;
 * the seekable API is not ported to the kernel, so it is omitted here. */
#include "../../include/linux/zxc.h"
 ...
 *
 * Uses the double-underscore spelling: the kernel headers define a `noinline`
 * macro (include/linux/compiler_attributes.h), which would otherwise expand
 * inside the attribute list and break the build.
 */
...
/* _Alignas is a C11 keyword handled by the compiler: no <stdalign.h> include
 * (the kernel does not ship that header). */
